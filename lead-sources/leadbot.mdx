...
- **Compatible with the rest of LeadTrackr** — shares the same [ChannelFlow cookies](/lead-sources/channel-flow-tracker#cookies-used-by-channelflow) and lead format, so the LeadBot runs side by side with the [GTM tag](/lead-sources/gtm) and the [WordPress plugin](/lead-sources/wordpress-forms)
...
| `launcher` | `true` | `false` hides the LeadBot launcher entirely, e.g. to run [interceptor-only](#whatsapp-interceptor) |
| `whatsappInterceptor` | `false` | `true` intercepts clicks on your site's existing WhatsApp links and opens the LeadBot flow instead ([see below](#whatsapp-interceptor)) |
...
| `defaultCountry` | auto | Preselected country in the WhatsApp phone step; defaults to the visitor's browser locale (`nl-BE` → Belgium), falling back to `NL`. No IP geolocation is used |
...
The message field is multi-line and grows with the content. On desktop, Enter sends and Shift+Enter adds a line break; on mobile the return key adds a line break and the send button sends — the same behavior as WhatsApp itself.
...
window.ltLeadBotConfig = {
  ...
  agentName: "Jane",
  agentPhoto: "https://example.com/jane.jpg"
};
```

When a visitor clicks a WhatsApp link, the LeadBot opens a modal (centered on desktop, bottom sheet on mobile) with the same WhatsApp chat flow: message → phone number → lead stored in LeadTrackr → WhatsApp opens with the message prefilled.
...
| Setup | Config |
|---|---|
| LeadBot only (default) | — |
| LeadBot + interceptor | `whatsappInterceptor: true` |
| Interceptor only, no launcher | `whatsappInterceptor: true, launcher: false` |

Interceptor leads arrive with the form name `LeadBot — WhatsApp Interceptor`, so you can tell them apart from leads that came through the LeadBot panel. The dataLayer events use `channel: "whatsapp"`, identical to the regular WhatsApp channel.
...
```html
<script src="https://cdn.jsdelivr.net/gh/leadtrackr/leadtrackr-leadbot@1.5.0/dist/lt-leadbot.min.js" async></script>
```

To check which version a page is running, look at the first line of the loaded bundle (`/*! LeadTrackr LeadBot v1.5.0 … */`) or the `x-jsd-version` response header.
