...
| `yeswetrack_…` | The phone number a visitor was shown | Set by call tracking | Call tracking script |
...
If you use LeadTrackr call tracking with dynamic number insertion, the swap script stores which phone number a visitor was shown in cookies starting with `yeswetrack_`. That is what allows an incoming call to be matched back to the right marketing source.

These cookies are written by the call tracking script, not by the Channel Flow Tracker or the plugin. They only appear on sites where call tracking is active.
